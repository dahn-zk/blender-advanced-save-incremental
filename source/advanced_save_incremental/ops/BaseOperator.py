...
class BaseOperator(bpy.types.Operator):
    ...
    #
    @classmethod
    def drawx(cls, layout: bpy.types.UILayout, **kwargs) -> Self:
        if cls.ui_icon is not None: kwargs.setdefault("icon", cls.ui_icon)
        if cls.ui_text is not None: kwargs.setdefault("text", cls.ui_text)
        return layout.operator(cls.bl_idname, **kwargs)
    #
    def _sorry_exception_message(self, why: str = ""):
        """ :( """
        return textwrap.dedent(f"""\
            Sorry, an unhandled error occured in the add-on {config.addon_qname}.
